...
tags:
  [
    ...
    "マクロ",
    "競技プログラミング",
    "競プロ用のライブラリを Rust で作ってみたシリーズ",
  ]
description: "dbg!の不満を解消、リリースビルドで無効化されるデバッグマクロを自作"
---

[Rust の dbg! マクロはリリースビルドでも普通に動く](/blog/rust-dbg-in-release/) で書いた通り、Rust の `dbg!` マクロはリリースビルドでも動き、多大なる実行時間を消費します。
...
[Rust の dbg! マクロはリリースビルドでも普通に動く](/blog/rust-dbg-in-release/) で書いたとおりです。レーティングを溶かす原因となったので恨んでいます。
