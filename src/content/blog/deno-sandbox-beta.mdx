...
description: "この半年間開発に携わってきたDeno Sandboxがついに公開。allowNetとsecrets機能について"
draft: true
---
...
<figure>
  <img src="/img/deno-sandbox-demo.png" alt="@deno/sandbox SDKを使ってSandboxを立ち上げ、lsコマンドとsshコマンドを実行している" />
...
最近の開発では、AIエージェントにコードを書かせることがもはや当たり前です。
...
[^sudo]: [sudo (すーどぅー)とは【ピクシブ百科事典】](https://dic.pixiv.net/a/sudo) にある通り、`sudo` の警告文として有名
...
<figure>
  <img src="/img/deno-sandbox-allow-net.png" alt="期待通り、example.com と developer.mozilla.org へのアクセスは成功し、maguro.devへのアクセスは失敗している" />
  <figcaption>期待通り、example.com と developer.mozilla.org へのアクセスは成功し、maguro.devへのアクセスは失敗している</figcaption>
</figure>
...
[^supplement-on-why-allow-net-does-not-protect-you]: 「AIエージェントにGitHubを操作させたい」という状況なので、上で説明した `allowNet` に `api.github.com` が設定済みと考えるのが自然です。つまり、このケースでは、`allowNet` 
による防御は効果がないと言えます
...
<figure>
  <img src="/img/deno-sandbox-secrets.png" alt="筆者のマシン上で echo $GITHUB_TOKEN して正しい値がセットされていることを確認したあと、上記のプログラムを実行し、Sandbox内では $GITHUB_TOKEN の値がダミーの値に置き換わっていて、しかし gh api user が成功している" />
</figure>
