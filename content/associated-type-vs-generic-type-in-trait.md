...
Rust でポリモーフィズムを実現するための中心的な機能ですが、他の場所で定義されている型（ここでは `Vec<i32>`）にメソッドを追加する、といったことも可能、というような紹介がされました。
...
ということは、さきほど出てきた `[u8; 4]` -> `IpAddr` の変換だけではなく、`[u16; 8]` -> `IpAddr` という変換も定義されているのが自然です。generic type であれば、これが可能です！
